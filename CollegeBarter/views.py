...
from collegebarter.models import *
...
def android_request(request):
    ...
    if type=='login':
        ...
        try:
            ...
            user=User.objects.filter(student_no=sno,school=school)
            if len(user)==0:
                rsdic={'info':'User does not exist','ret':1104}
                return
            elif user[0].is_register==1:
        ...
        try:
            ...
            if start==None and end==None:
                ...
                barters=Barter.objects.filter(userName=username)[start:end]
            barter_list=[]
            for barter in barters:
                dic={}
                dic['barterSha1']=barter.sha1
        ...
        try:
            ...
            if start==None and end==None:
                ...
                barters=Barter.objects.filter(category=category)[start:end]
            barter_list=[]
            for barter in barters:
                dic={}
                dic['barterSha1']=barter.sha1
...
def test(request):
    ...
    return HttpResponse(users[0].real_name)
